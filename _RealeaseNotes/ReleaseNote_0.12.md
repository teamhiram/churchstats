...
- 凡例トグルで系列を非表示にした際、バー上部の数字ラベルが全体合計のままだった問題を修正。
- 表示中の系列のみを合算した `visibleCount` を導出し、ラベルに反映するようにした。
...
| 対象 | 変更内容 |
|------|----------|
| `StatisticsCharts.tsx` | Recharts `<Legend>` を削除し、カスタムトグルボタン凡例を実装。`hiddenSeries` state で表示/非表示を管理。Y軸 domain に15%マージン追加。`chartData` で表示系列のみの合計を算出しラベルに反映。 |
...
- [ ] 系列を非表示にした際、バー上の数字が表示中の系列の合計のみになること
