...
* **報告者の所属地区:** 報告者は複数地区を担当可能。`reporter_districts` 等で「担当可能な地区」を保持。**メイン担当地区**を1つ設け、初期表示はその地区の統計画面とする。初期設定では**地方内の全地区にチェックが入った状態**とし、担当地区はチェックボックスで選択する。将来的に一部の地区を権限から除外する仕組みを用意する。
...
* **欠席アラート:** 過去(X)週間出席していた「ローカル」メンバーの今週欠席リストを表示。**X は初期設定 4、1〜52 をドロップダウンで選択可能**とする。
...
* **バックアップ・リストア:** データのバックアップとリストアを実装する。
* **UI言語:** 日本語のみでよい。
