...
} from "./sundayAttendanceColumns";
import { useSundayAttendanceRecordActions } from "./useSundayAttendanceRecordActions";
import { SundayAttendanceRosterView } from "./SundayAttendanceRosterView";
...
export function SundayAttendance({
  ...
  const { showLordsDayOnlineColumn, showLordsDayAwayColumn } = useDisplaySettings();
  const columnVisibility = useMemo(
    ...
    [showLordsDayOnlineColumn, showLordsDayAwayColumn]
  );
  const { tableColumnSpan, memoColumnSpan } = useMemo(
    () =>
      getSundayAttendanceColSpans({
        isEditMode,
        visibility: columnVisibility,
      }),
    [isEditMode, columnVisibility]
  );
...
const { data: guestData } = await supabase
  ...
  const {
    ...
    toggleIsAway,
    addFromSearch,
  } = useSundayAttendanceRecordActions({
    isEditMode,
    sundayIso,
    attendanceMap,
    memos,
            ...
            {loading ? (
              ...
              <>
                <SundayAttendanceRosterView
                  ...
                  guestIds={guestIds}
                  dirtyMemberIds={dirtyMemberIds}
                  showOnlineColumn={columnVisibility.online}
                  showAwayColumn={columnVisibility.away}
                  tableColumnSpan={tableColumnSpan}
                  memoColumnSpan={memoColumnSpan}
                  isSectionOpen={isSectionOpen}
                  toggleSectionOpen={toggleSectionOpen}
                  setAttendanceChoice={setAttendanceChoice}
                  toggleOnline={toggleOnline}
                  toggleIsAway={toggleIsAway}
                  setMemos={setMemos}
                  saveMemo={saveMemo}
