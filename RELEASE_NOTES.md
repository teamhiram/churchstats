...
- **年代（age_group）の拡張:** DB に Young（青年）・Middle（中年）・LateMiddle（壮年）・Senior（年長）を追加（マイグレーション 042）。既存の Adult は過渡期のため残し、アプリ側ラベルを「社会人(年代不詳)」に変更。
- **小組と地方の紐づけ:** `groups` に `locality_id` を追加（マイグレーション 043）。地方ラベルは localities を JOIN して取得。
- **中高生カテゴリ統一:** 中学生・高校生を ENUM 上で `junior_high`（中高生）に統一し、`high_school` を廃止（マイグレーション 044）。名簿・出欠・統計の各画面で対応。
- **バージョン表示:** ヘッダー・ログイン画面のバージョンバッジを 0.25.0 に更新。
...
- **ユーザー直接作成:** 設定 > ユーザー・ロール管理で、招待（メール送信）に加え、メールアドレス・パスワード・氏名を指定してユーザーを直接作成できるようにしました。SUPABASE_SERVICE_ROLE_KEY が必要です。
- **デフォルト表示地方（profiles.locality_id）:** サイトを開いたとき、Cookie が未設定なら profile の「デフォルト表示地方」を表示するようにしました。招待・直接作成時にデフォルト表示地方を選択でき、既存ユーザーもロール管理画面で編集・保存できます。
- **バージョン表示:** ヘッダーのバージョンバッジを 0.21.0 に更新。
